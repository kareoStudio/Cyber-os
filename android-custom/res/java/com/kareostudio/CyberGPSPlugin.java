...
@CapacitorPlugin(
    ...
    permissions = {
        ...
        @Permission(strings = {Manifest.permission.ACCESS_BACKGROUND_LOCATION}, alias = "bgLocation")
    }
)
public class CyberGPSPlugin extends Plugin {
    ...
    public void getLocation(PluginCall call) {
        if (!hasLocationPermission()) {
            requestAllPermissions(call, "locationCallback");
            return;
        }
    ...
    private void locationCallback(PluginCall call) {
        if (hasLocationPermission()) getCurrentLocation(call);
